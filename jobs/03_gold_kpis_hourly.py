...
def main():
    ...
    gold_kpis_df = gold_kpi_base_df.groupBy(F.window('event_ts','1 hour').alias('window'), 'campaign_id', 'ad_id') \
                  .agg(
                        ...
                        F.sum('cost_micros')/F.lit(1_000_000.0).alias('amount_spent'),
                        F.sum('revenue').alias('total_revenue'),
                   ) \
                  .withColumn('click_thru_rate', F.when(F.col('impressions') > 0,
                                                        F.col('clicks')/F.col('impressions'))
                                                        .otherwise(F.lit(0.0))) \
                  .withColumn('conversion_rate',F.when(F.col('clicks')>0,
                                                       F.col('conversions')/F.col('clicks'))
                                                       .otherwise(F.lit(0.0))) \
                  .withColumn('revenue_on_ad_spend', F.when(F.col('amount_spent')>0,
                                                            F.col('total_revenue')/F.col('amount_spent'))
                                                            .otherwise(F.lit(0.0))) \
                  .select(
    ...
    gold_kpis_df.writeStream.format('delta') \
                .outputMode('update') \
                .option('checkpointLocation', GOLD_CHK_KPIS) \
                .partitionBy('event_date') \
                .trigger(processingTime=TRIGGER) \
                .start(GOLD_KPIS_PATH)
